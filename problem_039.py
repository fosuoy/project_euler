...
def find_solutions_for_triangle(number, powers):
...
def find_most_results_for_range(upper_bound):
    ...
    result = 0
    result_length = 0
    test_range = [x for x in range(1, upper_bound) if x % 12 == 0]
    for number in test_range:
        solutions = find_solutions_for_triangle(number, powers_list)
